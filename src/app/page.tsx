...
import { BookOpen, DoorOpen, Plus, Sparkles } from "lucide-react";
import { playUiSound } from "@/lib/sound/ui-sounds";
import { GameRulesTutorial } from "@/components/tutorial/game-rules-tutorial";
...
export default function HomePage() {
  ...
  return (
    <>
        ...
        <section className="mx-auto grid min-h-[calc(100vh-2.5rem)] w-full max-w-6xl content-center gap-6 rounded-[8px] border-2 border-ink bg-petrol/92 p-4 shadow-pixel sm:p-6">
          <div className="mx-auto grid w-full max-w-4xl justify-items-center gap-3 text-center">
            ...
            <button
              ...
              className="mt-1 flex h-11 items-center gap-2 rounded-[6px] border-2 border-ink bg-bone px-3 font-display font-black text-ink shadow-card transition-transform hover:-translate-y-0.5 focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-2 focus-visible:outline-signal"
            >
              <BookOpen size={18} />
              Reglas del juego
            </button>
      ...
      <GameRulesTutorial open={tutorialOpen} onClose={() => setTutorialOpen(false)} />
    </>
  );
}
