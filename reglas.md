...
Se decide quién reparte primero (puede ser al azar o por sorteo).
3 Distribución de cartas
En la primera mano se reparten 5 cartas a cada jugador.

En las siguientes manos, el número de cartas cambia así:

5 → 4 → 3 → 2 → 1 → 2 → 3 → 4 → 5 → …
Se sigue este patrón hasta que se decida terminar el juego.
4 Apuestas (estimación de bazas)
Cada jugador mira sus cartas (excepto en la ronda de 1 carta; ver regla especial más abajo).
Cada jugador dice cuántas bazas cree que ganará en esa mano.
Regla especial del último jugador:
La suma de las bazas declaradas por todos los jugadores no puede ser igual al número de cartas repartidas.
Ejemplo: 5 cartas en mano; jugadores anteriores dicen 3, 2, 0 → último jugador no puede decir 5.
Esto asegura que siempre haya al menos un jugador que falle la predicción.
5 Cómo se juega una baza
...
Cada jugador pone su carta en la cabeza sin verla.
Solo los demás jugadores pueden verla.
Esto añade estrategia y sorpresa a la última baza de la “pirámide”.
7 Puntuación / sistema de vidas
Cada jugador empieza con 4 vidas.
...
Pierdes 3 vidas → “vas bur”
Pierdes 4 vidas →  “vas buru” → has perdido el juego
8 Orden de cartas (valor)
El valor de cartas sigue la lógica tradicional:
Baraja española: 1 (As) más alto, luego 12 (Rey), 11 (Sota), 10 (Caballo), etc.
Baraja francesa: As más alto, seguido por Rey, Reina, Jota, …
En caso de empate: hay dos modos de juego, o gana el que primero puso la cartas (reglas diego) o ganael que segundo puso la carta (reglas lete)
...
Sistema de vidas y frases: “vas b → vas bu → vas bur → vas buru”.
Patrón de cartas repartidas: 5→4→3→2→1→2→3→4→5.
